...
6 【Start Crawl Only, it is recommended to configure the upper-level proxy address in config.ini and link the passive scanner】
...
""")
        ...
        print("""Scan targets already in the scanner，Select the type to scan：
1 【Full Scan】
2 【Scan for high-risk vulnerabilities】
3 【Scan for XSS vulnerabilities】
4 【Scanning for SQL Injection Vulnerabilities】
5 【Weak Password Detection】
6 【Crawl Only, only crawlers are meaningless, it is recommended to configure the superior proxy address in config.ini and link the passive scanner】
7 【Scanning software scan】
9 【Only scan apache-log4j】 (please ensure that the current version supports log4j scanning, awvs 14.6.211220100 and above)
10 【Start Scanning for Bug Bounty High-Frequency Vulnerabilities】
11 【Scan for known vulnerabilities】 (common CVE, POC, etc.)
12 【Custom Template】
""")
    ...
    scan_label = str(input('Enter the asset tag to be scanned this time (empty):'))
    try:
        is_to_scan = True
        if target_scan==False:
            if '8'==scan_type:
        ...
        for target in targets:
            if target:
                ...
                if target_state[0]==1:
                    ...
                    print("{0} Added to scan queue ，first:".format(target),str(add_count_suss))
...
5 【High-risk vulnerability news push】 Enterprise WeChat robot
    """)
    ...
    if selection==1:
        ...
        push_wechat_group('The high-risk vulnerability message push has been enabled, and the script needs to be kept running in the foreground and will not be terminated.')
        message_push()
